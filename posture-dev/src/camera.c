...
// Frame callback for inference (set by camera_start)
static camera_frame_callback_t sg_frame_cb = NULL;
static void* sg_user_data = NULL;

// Binary conversion configuration (for monochrome displays)
...
/**
 * @brief Camera frame callback for RGB565 display with inference support
 * 
 * This callback is called when a new camera frame is available.
 * It:
 * 1. Calls inference callback if registered (for MoveNet processing)
 * 2. Converts YUV422 to RGB565 (using DMA2D if available)
 * 3. Handles rotation/swap if needed
 * 4. Flushes to display
 */
static OPERATE_RET __get_camera_raw_frame_rgb565_cb(TDL_CAMERA_HANDLE_T hdl, TDL_CAMERA_FRAME_T *frame)
{
    OPERATE_RET rt = OPRT_OK;
...
/**
 * @brief Camera frame callback for monochrome display with inference support
 */
...
{
    if (sg_tdl_camera_hdl != NULL) {
        PR_ERR("Camera already started");
        return OPRT_COM_ERROR;
    }

    // Register frame callback for inference
    sg_frame_cb = frame_cb;
    sg_user_data = user_data;

    // Initialize and start camera
    OPERATE_RET rt = __camera_init_internal();
    if (rt != OPRT_OK) {
        sg_frame_cb = NULL;
        sg_user_data = NULL;
        return rt;
    ...
    PR_NOTICE("Camera started with inference callback: %s", frame_cb ? "enabled" : "disabled");
    return OPRT_OK;
}
...
OPERATE_RET camera_deinit(void)
{
    ...
    PR_NOTICE("Camera module deinitialized");
    return OPRT_OK;
}
